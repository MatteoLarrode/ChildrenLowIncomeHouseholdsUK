...
```
There is data for a total of 340 local authorities covering 2016 to 2020, leading to 1700 observations across England, Wales and Scotland.
...
dataset_part1_cont <- dataset_part1_cont |> 
  ...
  mutate(UC_households_diff = UC_households_perc - lag(UC_households_perc)) |> 
  ungroup()

yearly_estimates_df <- dataset_part1_cont |> 
  group_by(year) |> 
  summarise(avg_UC_households_diff = mean(UC_households_diff, na.rm = TRUE),
            avg_0_16_pop = mean(population_0_16, na.rm = TRUE)) |> 
  mutate(est_increase_children_perc = avg_UC_households_diff * 0.29,
         est_increase_children_abs = est_increase_children_perc * avg_0_16_pop / 100)
  
# Average number of children in poverty in 
